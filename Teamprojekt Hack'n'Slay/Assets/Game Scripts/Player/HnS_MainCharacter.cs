...
public class HnS_MainCharacter : MonoBehaviour 
{
	...
	*/
	...
	void CheckGroundStatus()
	{
		...
		Debug.DrawLine(transform.position, transform.position  + (Vector3.down * m_GroundCheckDistance), Color.red, 1);
		#endif
		...
		//if(Physics.Raycast(transform.position, transform.position  + (Vector3.down * m_GroundCheckDistance)))
		if(m_totalGroundDistance > 0.1)
		{
			if(m_IsGrounded == true)
			{
				print ("isGrounded - doubleJump ok");
